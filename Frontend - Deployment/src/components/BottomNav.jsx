...
// Render the bottom nav component.
const BottomNav = ({ role }) => {
  // Get nav items.
  const getNavItems = () => {
    const studentItems = [
      { name: "Home", path: "/student-dashboard", icon: "bx-home" },
      { name: "Insights", path: "/student-insights", icon: "bx-line-chart" },
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span
                  className={`text-[clamp(0.68rem,2.1vw,0.74rem)] leading-none font-medium ${isActive ? "opacity-100" : "opacity-75"}`}
                >
                  {item.name}
                </span>
                {isActive && (
